import { createAsk, deleteAsk, getAsks, getMatter } from "../Database/router.js";
...
export function saveAskInDatabase() {
    ...
    buttonSave.addEventListener('click', async () => {
        ...
        await createAsk(params.get('matterid'), { ask: askValue.value, answer: answerValue.value, nivel: 10 });
        askValue.value = ''
        answerValue.value = ''
        modal.style.display = 'none'
    })
...
export function createTheExam(){
    ...
    buttonCrear.addEventListener('click', ()=>{
        ...
        window.location.href = `/pages/exam.html?id=267eh23g42343274curh&matterid=${params.get('matterid')}&qntd=${qntd}&fc=${isFlashCard}`
    });
}